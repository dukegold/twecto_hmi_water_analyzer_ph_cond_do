...
void IRAM_ATTR onTimer0_0(void);
...
String buf;
bool backlight = true, _wifiCon = false;
char str_rep[75];
...
void setup() {
  ...
  _wifiCon = (WiFi.status() == WL_CONNECTED);
  /*********************** End Intialize Wifi ****************************/
...
void loop(){
  ...
  if(millis() - wifiTime > WIFI_RETRY_MS) {
    ...
    if(_wifiCon && !mqttClient.connected()){
      mqttClient.connect(MQTT_CLIENT_ID,MQTT_USERNAME,MQTT_PASS);
    }
  }
  ...
  if (log_min) {
    ...
    saveData(cond_val,ph_val, temp_val, do_val, is_cond_con, is_ph_con, is_do_con, &rtc);
    if (_wifiCon){
...
void sensorRead(ModbusMaster *cond_sensor, ModbusMaster *ph_sensor, ModbusMaster *do_sensor,
  ...
  if (result == cond_sensor->ku8MBSuccess){
    for (int j = 0; j < 2; j++){
